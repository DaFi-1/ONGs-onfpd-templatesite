# ONGs-onfpd-templatesite

Site para uma ONG voltada a pessoas com autismo, desenvolvido em HTML, CSS e JavaScript. O projeto traz um design visual fragmentado em peças de quebra-cabeça, com animações que montam a página conforme a navegação. Criado em 3 dias com apoio de ChatGPT e OpenCode, incluindo imagens geradas por IA.

## 📋 Informações do projeto

**Status:** 🟢 Finalizado
**Stack:** HTML · CSS · JavaScript
**Conclusão:** 27/08/2026
**Desenvolvimento:** 3 dias

### 🛠️ Ferramentas utilizadas

- **ChatGPT** — Ideação, referências, geração de imagens e auxílio no desenvolvimento
- **OpenCode** — Desenvolvimento assistido por IA
Este é o meu segundo projeto social. A ideia deste projeto é criar um site para uma ONG voltada para pessoas com autismo. Pensei em fazer algo no estilo "fragmentado", já que o objetivo não é apenas criar um site funcional, mas desenvolver um site temático, visualmente interessante e persuasivo para quem acessá-lo pela primeira vez.

Este projeto também é um desafio pessoal. Não quero apenas fazer mais um site, mas experimentar uma abordagem diferente de design e interação, tentando transmitir a mensagem da ONG através da própria experiência de navegação.

Atualmente, estou montando minhas ideias com o ChatGPT, buscando referências e ideias criativas para o tipo de contribuição que, inicialmente, eu não tinha a mínima ideia de como desenvolver.

---

## O início da ideia

Depois de vários prompts e experimentações, cheguei a alguns layouts que começaram a representar melhor o que eu estava buscando.

Esses dois modelos refletem bem a mensagem visual que quero transmitir:

<img width="1536" height="1024" alt="image" src="https://github.com/user-attachments/assets/396698cb-bced-4c89-ac62-74002663afb4" />

<img width="950" height="1655" alt="image" src="https://github.com/user-attachments/assets/7fc6f83f-bb08-4703-a820-549f8328dc1b" />

Além disso, pensei em fazer com que o site fosse sendo construído conforme o usuário rolasse a página.

A ideia é que os elementos apareçam, se encaixem e formem a composição gradualmente durante a navegação.

Acredito que seja uma boa ideia para testar. Se não funcionar, é só descartar tudo e tentar outra abordagem.

A IA já cuida da parte mais chata do desenvolvimento, então não faz sentido eu ter um vínculo afetivo com o código. Posso simplesmente experimentar, errar, descartar e começar novamente.

E, sinceramente, isso é uma das coisas mais interessantes desse processo.

---

## O primeiro grande prompt

Depois de algumas ideias, pedi ao ChatGPT para gerar um único prompt, sem economizar palavras, para o OpenCode gerar o início do projeto.

A ideia era realmente testar até onde um único prompt conseguiria levar o desenvolvimento.

As etapas definidas inicialmente foram:

- [x] Definir stack técnica e estrutura do projeto
- [x] Criar design system (CSS variables, tokens, base styles)
- [ ] Implementar componentes reutilizáveis (Header, PuzzlePiece, BrushStroke, etc.)
- [ ] Implementar Hero com composição visual fragmentada
- [ ] Implementar seção de Pilares
- [ ] Implementar seção Sobre o Projeto
- [ ] Implementar seção Impacto com contadores animados
- [ ] Implementar Citação/Manifesto
- [ ] Implementar seção Projetos
- [ ] Implementar Galeria
- [ ] Implementar seção Faça Parte
- [ ] Implementar seção Notícias
- [ ] Implementar CTA de Doação
- [ ] Implementar Footer completo
- [ ] Implementar acessibilidade completa
- [ ] Implementar responsividade mobile
- [ ] Implementar animações e microinterações
- [ ] SEO e performance
- [ ] Testes e ajustes finais

---

## Menos framework, mais necessidade

Durante esse processo, fiz algumas exigências porque a IA estava começando a exagerar na arquitetura do projeto.

Decidi exigir:

- HTML puro
- CSS puro
- JavaScript puro

Um projeto desse tamanho não precisa necessariamente de toda a complexidade que estava sendo proposta.

Eu poderia utilizar frameworks, bibliotecas e diversas outras ferramentas, mas, neste caso, isso acabaria adicionando mais coisas do que eu realmente preciso.

No fim das contas, estou reinventando a roda.

Porém, com IA, consigo reinventar essa roda em segundos e moldá-la exatamente às minhas necessidades.

E isso é uma maravilha.

É, inclusive, uma das partes mais satisfatórias desse projeto.

---

## Primeira versão

O resultado inicial ficou até legal:

<img width="1254" height="638" alt="image" src="https://github.com/user-attachments/assets/88a4ac30-4a0e-4111-84c9-8e2ffa3cecf4" />

Mas não era exatamente o que eu tinha em mente.

Então, decidi não tentar consertar tudo de uma vez.

Vou fazer prompt por prompt até chegar ao resultado que realmente desejo.

A ideia aqui não é apenas fazer a IA gerar código.

Quero usar a IA como uma ferramenta de experimentação para descobrir qual direção visual funciona melhor.

---

## A primeira evolução

Depois de algumas alterações, finalmente cheguei a um formato que considero muito mais próximo da minha ideia para o site.

<img width="851" height="426" alt="image" src="https://github.com/user-attachments/assets/c2497a6c-913c-4185-980b-953cce2d36d6" />

Agora sim começou a aparecer a identidade que eu estava procurando.

E essa fonte Nerd Font é simplesmente sensacional.

---

## Segunda parte

A segunda parte ficou exatamente como eu queria.

A ideia é trabalhar com uma divisão visual que mantenha o conceito fragmentado do projeto.

<img width="869" height="248" alt="image" src="https://github.com/user-attachments/assets/1467b7c2-7796-4e70-8496-38fcdfe7027c" />

Para o banner, provavelmente vou utilizar uma sequência de peças de quebra-cabeça em diferentes tons de verde.

A intenção é continuar levando o conceito do quebra-cabeça para diferentes partes do site, sem deixar que ele pareça apenas um elemento decorativo.

---

## Seção de apresentação

Essa parte da apresentação ficou muito boa.

Finalmente consegui chegar ao resultado visual que estava imaginando desde o início.

<img width="907" height="510" alt="image" src="https://github.com/user-attachments/assets/1095ff2b-e4fd-4601-80f0-a49e45ab1b50" />

A partir daqui, a ideia é continuar construindo o restante do site seguindo essa mesma linguagem visual.

---

## Seção de agradecimentos

Na seção de agradecimentos, já tenho algo em mente, então vai ser muito fácil para a IA implementar.

A ideia é ter um texto à esquerda e uma imagem gerada por IA à direita. Gostei bastante do resultado.

<img width="871" height="418" alt="image" src="https://github.com/user-attachments/assets/cfa876e9-ded4-472d-8866-1a794ae6f094" />

---

## Última parte

Para finalizar, há o número de telefone da própria ONG. Como se trata de um dado público da organização, não há problema em utilizá-lo.

Mesmo assim, não vou mostrá-lo aqui.

Também não vou mostrar a seção da galeria para evitar expor as imagens no GitHub. Mesmo sendo imagens públicas, prefiro não reproduzi-las nesta documentação.

<img width="881" height="256" alt="image" src="https://github.com/user-attachments/assets/0556bf22-0be2-4ff6-8d87-b3f349fffabc" />

---

## Como executar

Como se trata de um site estático, basta abrir o arquivo `index.html` diretamente no navegador ou subir um servidor local:

```bash
python3 -m http.server 8000
```

Depois, é só acessar `http://localhost:8000`.

---

# Considerações finais

O projeto foi finalizado em 3 dias, mas cerca de 90% dele já havia sido desenvolvido em aproximadamente 9 horas. O restante demorou mais por causa dos feedbacks e ajustes solicitados pelo cliente.

Nada contra o cliente, e não levem isso para o lado errado: isso é apenas parte da documentação do processo.

Para desenvolver o projeto, utilizei apenas:

- HTML
- CSS
- JavaScript

Não foi necessário utilizar nada além disso.

Também utilizei o ChatGPT em conjunto com o OpenCode durante todo o processo.

Todas as imagens utilizadas no projeto foram geradas pelo ChatGPT, que fez um excelente trabalho nesse aspecto.

No final, o projeto foi concluído com sucesso.

Projeto finalizado.






















